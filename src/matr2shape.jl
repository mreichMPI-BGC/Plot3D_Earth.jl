using GeometryBasics, LinearAlgebra, GLMakie, CairoMakie
...
function brick2movie(brick, typ::Symbol=:sphere; nsubsteps=4, nRounds=10,
    ...
    #rot = Observable(π)
    if typ == :sphere
        #MyMakie.rotate!(axis.scene, Vec3f(0, 0, 1), π)
        MyMakie.zoom!(axis.scene, 0.7)
        rotate_cam!(axis.scene, (1.1π, 0.5π, 0.0))

    else
        #MyMakie.rotate!(axis.scene, Vec3f(0, 0, 1), 0)
        MyMakie.zoom!(axis.scene, 0.7)
        rotate_cam!(axis.scene, (-360/16 / 360 * 2π, 225 / 360 * 2π,- 0.0))
    end

    #rotate_cam!(axis.scene, (0.5,0,0))
    #MyMakie.rotate!(Accum, axis.scene, Vec3f(0, 1, 0), π/4 )
    ...
    i = 1
    #anim = @time MyMakie.record(fig, outfile, list, framerate=16) do e
    anim = @time MyMakie.Record(fig, list, framerate=16) do e
        idx[] = e.layer
        relSubStep[] = e.sub / nsubsteps
        # MyMakie.rotate!( axis.scene, Vec3f(0, 0, 1), π + 2π*(e.r*nlayers*nSubsteps + (e.layer-1) * nSubsteps + e.sub )/(nlayers*nSubsteps*nRounds) )
        #if typ == :sphere
            rotate_cam!(axis.scene, (0.0, -(θ_all[i] - θ_all[max(i-1,1)]) / 360 * 2π, 0.0))
            rotate_cam!(axis.scene, ((ϕ_all[i] - ϕ_all[max(i-1,1)]) / 360 * 2π, 0, 0))
        #end

        #MyMakie.rotate!(Accum,  axis.scene, Vec3f(0, 0, 1), (θ_all[i+1] - θ_all[i])/360*2π )
        #MyMakie.rotate!(Accum, axis.scene, Vec3f(0, 1, 0), ϕ_all[i]/360*2π )
        #println(θ_all[i], " ", e)
        i += 1
        #MyMakie.rotate!(fig.content[1].scene, Vec3f(0, 0, 1), i*π/48)
    end
    display(fig)
    save(outfile, anim)
    anim
...
function exampleSeasonal()
    ds = nc.Dataset(joinpath(@__DIR__, "..", "data", "GPP_MR.msc.nc"))
    gpp=@chain begin
        ds[:variable][:]
        coalesce.(NaN32)
        reverse(dims=2)
    ...
    plot(s::Symbol; kw...) = brick2movie(gpp, s; nRounds=10, nsubsteps=4,  elev=dem05deg, overlay=countryRaster(;res=0.25f0), smoothWin=Kernel.gaussian(3),
        pal, title, legLabel, layerTitle, outfile="GPP025_seas_video_$s.mp4", kw...)

    plot(:sphere, nRounds=1, nsubsteps=2, theta_traj=[0.0,0.0])
   # plot(:flat)
    #plot(:robin)
end
